...
async function loadPoemData(poemId) {
    ...
    try {
        ...
        if (poemSnap.exists()) {
            ...
            window.showToastNotification(`கவிதை "${poem.title.substring(0, 20)}..." வெற்றிகரமாக ஏற்றப்பட்டது.`, 'success');
...
// --- 3. COMMENT SYSTEM (Logic remains the same) ---
...
window.ratePoem = async function(ratingValue) {
    // Logic for 5-star rating system (Firestore setDoc)
    window.showToastNotification(`மதிப்பீடு அளிக்கப்பட்டது: ${ratingValue} நட்சத்திரங்கள்.`, 'success');
    // NOTE: Full logic requires user authentication and Firestore transaction.
}

window.handleReaction = async function(reactionType) {
    // Logic for like/reaction system (Firestore transaction/update)
    window.showToastNotification(`ரியாக்ஷன் (${reactionType}) பதியப்படுகிறது...`, 'info');
    // NOTE: Full logic requires user authentication and Firestore transaction.
}

window.toggleBookmark = async function() {
    // Logic for bookmark/save for later (Firestore setDoc/deleteDoc)
    window.showToastNotification("கவிதை சேமிக்கப்பட்டது!", 'success');
    // NOTE: Full logic requires user authentication and Firestore interaction.
}
